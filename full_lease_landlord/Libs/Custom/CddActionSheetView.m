...
- (instancetype)initWithOptions:(NSArray<NSString *> *)titles completion:(void (^)(NSInteger index))completion cancel:(void (^)(void))cancelBlock {
    self = [super init];
    if (self) {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        if (_title && ![_title isEqualToString:@" "]) {
            ...
            line.backgroundColor = RGBA(195, 195, 195, 0.3);
...
//    if (indexPath.section == self.titlesArray.count - 1 && indexPath.row == self.titlesArray[indexPath.section].count - 1) {
//        if (iPhoneX) {
//           return self.itemHeight + Bottom_Height_Dif;
//        }
//    }
