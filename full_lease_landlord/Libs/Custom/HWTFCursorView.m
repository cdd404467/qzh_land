...
- (void)tfEditingChanged:(UITextField *)textField
{
    ...
    if (textField.text.length >= self.itemCount) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(verificationCodeInputIsComplete:)]) {
            [self.delegate verificationCodeInputIsComplete:textField.text];
        }
    }
}
...
- (void)setupView
{
    ...
    cursorView.backgroundColor = HEXColor(@"28C2CE", 1);
    cursorView.alpha = 0;
