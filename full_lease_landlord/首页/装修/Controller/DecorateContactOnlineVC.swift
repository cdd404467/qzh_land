...
class DecorateContactOnlineVC: BaseViewController {
...
extension DecorateContactOnlineVC {
    ...
    @objc func checSMSkCodeAndSubmit() {
        ...
        if nameTF.text?.count == 0 {
            ...
            CddHud.showTextOnly("请输入正确的手机号", view: view)
            return;
        } else if codeTF.text?.count == 0 {
            CddHud.showTextOnly("请输入验证码", view: view)
            return;
        }
        
        let dict: [String: Any] = [
            "userPhone":phoneTF.text! as String,
            "status":10,
            "SMSCode":codeTF.text ?? "",
            "name":nameTF.text ?? ""
        ]
        
        NetChain.parameters(dict).request(url: "member/Note/verifyNote").success {[weak self] json in
            if json["code"].intValue == 200 {
